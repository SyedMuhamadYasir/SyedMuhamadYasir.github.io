...
```bash
python 23_window_descriptive_checks.py --tx-file /ABS/PATH/TO/current_48d_transactions.csv --labels-csv /ABS/PATH/TO/labeled_nodes_in_graph.csv --out-dir ./window_descriptive_checks
```
...
Do **not** hand-pick random files unless something broke.
Just send the folders below.

### Always send these folders
- `OUT_ROOT/step7_holdout/final_test/`
- `OUT_ROOT/step8_review_assets/`

### If outer robustness was run, also send
- `OUT_ROOT/outer_sensitivity/`

### If risk score was run, also send
- `OUT_ROOT/risk_score/`

### If descriptive time-window checks were run, also send
- `./window_descriptive_checks/` if you used the command above exactly
- or the exact folder passed to `--out-dir`
