...

{
  "windows": [
    {
      "window_id": "submitted_48d",
      "start_date": "2024-12-30",
      "end_date": "2025-02-15",
      "graph_path": "/abs/path/to/LCC_G_tx_undirected_weighted.pkl",
      "labels_csv": "/abs/path/to/labeled_nodes_in_graph.csv"
    },
    {
      "window_id": "earlier_48d",
      "start_date": "2024-11-12",
      "end_date": "2024-12-29",
      "graph_path": "/abs/path/to/earlier/LCC_G_tx_undirected_weighted.pkl",
      "labels_csv": "/abs/path/to/earlier/labeled_nodes_in_graph.csv",
      "build_commands": [
        "python build_window_graph.py --start-date 2024-11-12 --end-date 2024-12-29 --out-root /abs/path/to/earlier"
      ]
    },
    {
      "window_id": "later_48d",
      ...
      "end_date": "2025-04-04",
      "graph_path": "/abs/path/to/later/LCC_G_tx_undirected_weighted.pkl",
      "labels_csv": "/abs/path/to/later/labeled_nodes_in_graph.csv"
    }
  ]
}
...
def run_branch(branch: str, labels_csv: Path, graph_path: Path, out_dir: Path, min_class_size: int) -> None:
    df_bin, df_mul, keep_classes, merged_classes = load_branch_frames(labels_csv, min_class_size=min_class_size)
    branch_df = df_bin if branch == "binary" else df_mul
    dev_df, test_df = stratified_split(branch_df, test_ratio=TEST_RATIO, seed=RANDOM_SEED)

    branch_dir = out_dir / branch
    branch_dir.mkdir(parents=True, exist_ok=True)
    write_df(dev_df, branch_dir / f"fixed_dev_labels_{branch}.csv")
    write_df(test_df, branch_dir / f"fixed_test_labels_{branch}.csv")
    ...
    for repeat_id, train_df, val_df, seed in make_dev_repeats(
        ...
        repeat_dir = branch_dir / repeat_id
        pred_df = run_sslpa_from_train_df(
            train_df, mode=branch, out_csv=repeat_dir / f"{repeat_id}_train_labels_{branch}.csv", graph_path=graph_path
        )
        raw_metrics, grid_df, selected = tune_threshold_grid(pred_df, val_df, mode=branch)
        selected["repeat_id"] = repeat_id
        selected["seed"] = seed
    ...
    final_threshold = choose_final_threshold_from_selected(selected_rows, mode=branch)
    save_json(branch_dir / f"final_threshold_{branch}.json", final_threshold)
    final_results = final_eval(dev_df, test_df, branch, final_threshold, branch_dir, graph_path)

    if branch == "binary":
        overlap = compute_risky_overlap(test_df, final_results["ncrf_pred"])
        write_df(pd.DataFrame([overlap]), branch_dir / "final_test_risky_overlap_summary.csv")

    save_json(branch_dir / "branch_manifest.json", {
        "branch": branch,
        "labels_csv": str(labels_csv),
        "graph_path": str(graph_path),
        "n_repeats": N_REPEATS,
        "val_ratio_within_dev": VAL_RATIO_WITHIN_DEV,
        "test_ratio": TEST_RATIO,
        "multiclass_keep_classes": keep_classes if branch == "multiclass" else None,
        "multiclass_merged_classes": merged_classes if branch == "multiclass" else None,
    })


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--manifest", default="window_sensitivity_manifest.example.json")
    parser.add_argument("--out-dir", default=str(WINDOW_SENSITIVITY_DIR))
    parser.add_argument("--run-build-commands", action="store_true")
    ...
    for window in manifest.get("windows", []):
        ...
        window_dir = out_dir / window_id
        window_dir.mkdir(parents=True, exist_ok=True)

        graph_path = Path(window["graph_path"])
        labels_csv = Path(window["labels_csv"])

        if args.run_build_commands and (not graph_path.exists() or not labels_csv.exists()):
            for cmd in window.get("build_commands", []):
                subprocess.run(cmd, shell=True, check=True)

        if not graph_path.exists():
            raise FileNotFoundError(f"Missing graph_path for window {window_id}: {graph_path}")
        if not labels_csv.exists():
            raise FileNotFoundError(f"Missing labels_csv for window {window_id}: {labels_csv}")

        save_json(window_dir / "window_manifest.json", window)

        run_branch("binary", labels_csv, graph_path, window_dir, args.min_class_size)
        run_branch("multiclass", labels_csv, graph_path, window_dir, args.min_class_size)

        # support counts
        b_test = pd.read_csv(window_dir / "binary" / "fixed_test_labels_binary.csv")
        m_test = pd.read_csv(window_dir / "multiclass" / "fixed_test_labels_multiclass.csv")
        summary_rows.append({
            "window_id": window_id,
            "start_date": window.get("start_date"),
            "end_date": window.get("end_date"),
            "binary_test_size": len(b_test),
            "binary_test_scam": int((b_test["label"] == "SCAM").sum()),
            "multiclass_test_size": len(m_test),
            "multiclass_unique_test_labels": int(m_test["label"].nunique()),
            "graph_path": str(graph_path),
            "labels_csv": str(labels_csv),
        })

    write_df(pd.DataFrame(summary_rows), out_dir / "window_manifest.csv")
    print(f"Done: window sensitivity artifacts saved under {out_dir}")
